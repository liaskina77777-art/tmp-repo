...
async def _upload_single_file(session: aiohttp.ClientSession, file_path: Path, token: str) -> None:
    #Загружает один файл любого формата на сервер с использованием токена
    ...
    # Вариант 1: Пробуем получить URL через getURLForDownload с правильным форматом данных
    try:
        #  формат: [{}, "имя_файла"]
...
def func(file_or_dir_path: str) -> None:
